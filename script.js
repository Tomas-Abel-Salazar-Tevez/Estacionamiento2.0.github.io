...
function agregarAuto() {
  ...
  if (autos.some(a => a.matricula === matricula)) return alert("Esa matrícula ya está registrada.");
  const entrada = Date.now();
  autos.push({ matricula, entrada, tipo: 'auto' });
  guardar();
  renderizar();
...
function agregarMoto() {
  ...
  if (autos.some(a => a.matricula === matricula)) return alert("Esa matrícula ya está registrada.");
  const entrada = Date.now();
  autos.push({ matricula, entrada, tipo: 'moto' });
  guardar();
  renderizar();
  document.getElementById('matricula').value = "";
}
...
function listoSeFue(index) {
  ...
  if (auto.tipo === 'moto') {
    ...
    if (tiempo <= 5) {
      ...
      const extras = Math.ceil((tiempo - 72) / 30);
...
function renderizarAutos() {
  ...
  autos.forEach((auto, i) => {
    ...
    if (auto.tipo === 'moto') {
      ...
      div.classList.add('auto-clase');
    }

    const tiempo = Math.ceil((Date.now() - auto.entrada) / 60000);
    if (tiempo >= 35 && tiempo <= 37 && auto.tipo === 'auto') {
      div.classList.add('alerta-rojo');
    }

    const icono = auto.tipo === 'moto' ? '🛵' : '🚗';
    const info = document.createElement('div');
    info.className = 'info';
    info.innerHTML = `<strong>${icono} ${auto.matricula}</strong><br>Entrada: ${new Date(auto.entrada).toLocaleTimeString()}<br><span id="cronometro-${i}"></span>`;
  ...
  historialFiltrado.forEach(dia => {
    ...
    dia.autos.forEach((h, index) => {
      ...
      row.innerHTML = `
        ...
        <td>${h.tiempo} min</td>
        <td>$${h.precio}</td>
        <td><button class="eliminar" onclick="eliminarHistorial('${dia.fecha}', ${index})">❌</button></td>`;
...
async function generarPDF() {
  ...
  const historialFiltrado = historial.filter(dia => {
    ...
    if (rangoDesde && fechaDia < new Date(rangoDesde)) return false;
    if (rangoHasta && fechaDia > new Date(rangoHasta)) return false;
    return true;
  });
